...
oneTimeSetUp()
{
  ...
  __TEST_TEMP_DIR="${USHUNIT_TEMP_DIR}/test_tar_data"
...
test_tar_data_no_such_file_fail()
{
  if commandExists "tar"; then
    assertFalse "_tar_data \"${__TEST_TEMP_DIR}/no_such_file.txt\" \"${__TEST_TEMP_DIR}/no_such_file_fail.tar\""
  else
    skipTest "no such file or directory: 'tar'"
  fi
}

test_tar_data_success()
{
  __UAC_OPERATING_SYSTEM="${TEST_SYSTEM_OS:-}"

  if commandExists "tar"; then
    if [ -n "${__UAC_OPERATING_SYSTEM}" ]; then
      assertTrue "_tar_data \"${__TEST_TEMP_DIR}/file_list.txt\" \"${__TEST_TEMP_DIR}/success.tar\""
      assertFileExists "${__TEST_TEMP_DIR}/success.tar"
    else
